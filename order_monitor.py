...
class OrderMonitor:
    ...
    def __init__(self):
        ...
        # 🔥 ИСПРАВЛЕНИЕ: Отслеживание отправленных уведомлений
        self.notified_orders = set()  # order_id уже уведомленных ордеров
    ...
    def add_order_group(self, order_result: Dict) -> None:
        ...
        try:
            ...
            order_group = {
                ...
                'main_status': 'NEW',  # 🔧 LIMIT ордер может быть не исполнен сразу!
                'stop_status': 'NEW',
                'tp_status': 'NEW',
    ...
    def _handle_filled_order(self, order_group: Dict, order_id: str, order_data: Dict) -> None:
        ...
        try:
            ...
            # 🔥 ИСПРАВЛЕНИЕ: Проверяем, не уведомляли ли уже об этом ордере
            notification_key = f"{order_id}_filled"
            if notification_key in self.notified_orders:
                return  # Уже уведомляли, не дублируем
            
            # Определяем тип ордера
            if order_id == order_group['main_order_id']:
                order_type = "MAIN"
            elif order_id == order_group['stop_order_id']:
                order_type = "STOP"
            elif order_id == order_group['tp_order_id']:
                order_type = "TAKE_PROFIT"
            else:
                return  # Неизвестный ордер
            
            # Добавляем в уведомленные ПЕРЕД отправкой
            self.notified_orders.add(notification_key)
            
            if order_type == "MAIN":
                # 🔧 НОВОЕ: Исполнился основной ордер - позиция открыта!
                logger.info(f"✅ ОСНОВНОЙ ОРДЕР исполнен для {ticker} - позиция открыта!")
                
                # Обновляем статус
                order_group['main_status'] = 'FILLED'
                
                # Отправляем уведомление о входе в позицию
                self._send_position_opened_notification(order_group, order_data)
                # НЕ удаляем группу - продолжаем следить за SL/TP!
    ...
    def _handle_canceled_order(self, order_group: Dict, order_id: str, order_data: Dict) -> None:
        ...
        try:
            ...
            # 🔥 ИСПРАВЛЕНИЕ: Проверяем, не уведомляли ли уже об отмене этого ордера
            notification_key = f"{order_id}_canceled"
            if notification_key in self.notified_orders:
                return  # Уже уведомляли, не дублируем
            ...
            if order_type == "MAIN":
                # 🔧 НОВОЕ: Основной ордер отменен - отменяем SL/TP!
                logger.warning(f"❌ ОСНОВНОЙ ОРДЕР отменен для {ticker}")
                
                # Отменяем связанные ордера
                stop_order_id = order_group['stop_order_id']
                tp_order_id = order_group['tp_order_id']
                
                self._cancel_order(ticker, stop_order_id, "Основной ордер отменен")
                self._cancel_order(ticker, tp_order_id, "Основной ордер отменен")
                
                # Отправляем уведомление
                self._send_main_order_canceled_notification(order_group, order_data)
                ...
                # Отправляем уведомление о внешней отмене ТОЛЬКО ОДИН РАЗ
    ...
    def _remove_order_group(self, order_group: Dict) -> None:
        ...
        try:
            ...
            # 🔥 ИСПРАВЛЕНИЕ: Очищаем уведомления для этой группы
            notifications_to_remove = []
            for notification_key in self.notified_orders:
                if (main_order_id in notification_key or 
                    stop_order_id in notification_key or 
                    tp_order_id in notification_key):
                    notifications_to_remove.append(notification_key)
            
            for key in notifications_to_remove:
                self.notified_orders.discard(key)
...
"""
    ...
    def _send_position_opened_notification(self, order_group: Dict, order_data: Dict) -> None:
...
💰 <b>Количество:</b> {quantity}
💵 <b>Цена входа:</b> {fill_price:.6f}

🎯 Stop Loss и Take Profit активны

⏰ {datetime.now().strftime('%H:%M:%S')}
"""
            
            telegram_bot.send_message(message)
            logger.info(f"📱 Уведомление об открытии позиции {ticker} отправлено")
            
        except Exception as e:
            logger.error(f"❌ Ошибка уведомления об открытии позиции: {e}")
    
    def _send_main_order_canceled_notification(self, order_group: Dict, order_data: Dict) -> None:
        """Отправляет уведомление об отмене основного ордера"""
        try:
            ticker = order_group['ticker']
            signal_type = order_group['signal_type']
            quantity = order_group['quantity']
            ...
            message = f"""
❌ <b>ОСНОВНОЙ ОРДЕР ОТМЕНЕН</b> ❌

📊 <b>Символ:</b> {ticker}
📈 <b>Направление:</b> {signal_type}
💰 <b>Количество:</b> {quantity}

🛡️ Stop Loss и Take Profit автоматически отменены

⚠️ Позиция не была открыта

⏰ {datetime.now().strftime('%H:%M:%S')}
"""
            
            telegram_bot.send_message(message)
            logger.info(f"📱 Уведомление об отмене основного ордера {ticker} отправлено")
            
        except Exception as e:
            logger.error(f"❌ Ошибка уведомления об отмене основного ордера: {e}")
