...
# Локальные импорты
from config import (
    BINANCE_API_KEY, BINANCE_API_SECRET, NETWORK_MODE, FUTURES_LEVERAGE, RISK_PERCENT
)
...
def main():
    ...
    def get_human_report(report_type, monitor):
        buf = StringIO()
    ...
    try:
        ...
        # Всегда формируем для Telegram только баланс
        monitor.print_balance_summary()
        balance_text = get_human_report('balance', monitor)

        # Интерактивное подтверждение на отправку в Telegram
        if balance_text:
            print("\nОтправляем отчет в Telegram? y/N:", end=' \u001b[33m')
            ...
            if choice == 'y':
                try:
                    ...
                    tg = TelegramBot()
                    # Ограничим длину сообщения для Telegram (4096 символов)
                    if len(balance_text) > 4000:
                        balance_text = balance_text[:4000] + "... (truncated)"
                    tg.send_message(balance_text, parse_mode="HTML")
                    print("✅ Отчет отправлен в Telegram!")
                except Exception as e:
                    print(f"❌ Ошибка отправки в Telegram: {e}")
            else:
                print("⏩ Отправка в Telegram отменена.")
    except Exception as e:
