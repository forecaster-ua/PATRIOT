...
class EnhancedSignalProcessor:
    ...
    def place_full_order_setup(self, signal_data: Dict) -> Dict:
        ...
        try:
            ...
            # Проверяем режим позиций для фьючерсов
            ...
            if config.BINANCE_TESTNET:
                try:
                    ...
                    logger.info(f"📊 Текущий режим позиций: {'Hedge' if is_hedge_mode else 'One-way'}")
                    
                    if is_hedge_mode:
                        use_position_side = True
                        logger.info("� Используем Hedge режим с positionSide")
                    else:
                        logger.info("💡 Используем One-way режим без positionSide")
                        
                except Exception as e:
                    logger.warning(f"⚠️ Ошибка проверки режима позиций: {e}")
                    logger.info("💡 Используем One-way режим по умолчанию")
                    use_position_side = False
            
            # Определяем сторону ордера
            side = Client.SIDE_BUY if signal_data['signal'] == 'LONG' else Client.SIDE_SELL
            position_side = 'LONG' if signal_data['signal'] == 'LONG' else 'SHORT'  # Для Hedge режима
            ...
            if config.BINANCE_TESTNET:
                # Для testnet используем futures API
                if use_position_side:
                    # Hedge режим - с positionSide
                    ...
                    # One-way режим - без positionSide
                    entry_order = self.binance_client.futures_create_order(
                        symbol=self.ticker,
                        side=side,
                        type=Client.ORDER_TYPE_MARKET,
